...
def run_full_pipeline(_uploaded_eeg_file, _uploaded_survey_file, _config):
    ...
    if not all_trials:
        return None, None, None, "有効な試行データの読み込みに失敗しました。ファイル形式やIDのマッチングを確認してください。"
        ...
        if processed_trials:
            ...
            with tabs[0]:
                ...
                if subjects_raw:
                    ...
                    if trials_for_subject_raw:
                        selected_trial_id_raw = st.selectbox("試行を選択", [t.trial_id for t in trials_for_subject_raw], key="raw_trial_selector")
            ...
            with tabs[1]:
                ...
                if subjects_qc:
                    ...
                    if valid_trials:
                        selected_trial_id_qc = st.selectbox("試行を選択", [t.trial_id for t in valid_trials], key="qc_trial_selector")
...
# --- フッター ---
st.markdown("---")
st.markdown("<div style='text-align: center; color: #888;'>🧠 EEG画像嗜好解析システム v1.4 (Stable)</div>", unsafe_allow_html=True)
